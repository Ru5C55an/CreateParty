...
class SetupProfileViewController: UIViewController {
    ...
    @objc private func nextButtonTapped() {
        guard Validators.isFilled(username: fullNameLabel.text, description: aboutMeTextField.text, sex: sexSegmentedControl.titleForSegment(at: sexSegmentedControl.selectedSegmentIndex), birthday: birtdayLabel.text) else {
            self.showAlert(title: "Ошибка", message: "Не все поля заполнены")
            return
        }
        present(SecondSetupProfileViewController(currentUser: currentUser, username: fullNameTextField.text!, description: aboutMeLabel.text!, birthday: birtdayLabel.text!, sex: sexSegmentedControl.titleForSegment(at: sexSegmentedControl.selectedSegmentIndex)!), animated: true, completion: nil)
    }
}
