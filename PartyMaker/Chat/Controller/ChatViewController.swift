...
class ChatViewController: MessagesViewController {
    ...
    private let chat: PChat
    
    init(user: PUser, chat: PChat) {
        self.user = user
        self.chat = chat
    ...
    private func insertNewMessage(message: PMessage) {
    ...
    private func sendImage(image: UIImage) {
        StorageService.shared.uploadImageMessage(photo: image, to: chat) { (result) in
            ...
            case .success(let url):
                var message = PMessage(user: self.user, image: image)
...
extension ChatViewController {
    ...
    func configureSendButton() {
        ...
        messageInputBar.sendButton.applyGradients(cornerRadius: 10, from: .bottomLeading, to: .topTrailing, startColor: #colorLiteral(red: 0.05098039216, green: 0.5647058824, blue: 0.9137254902, alpha: 1), endColor: #colorLiteral(red: 0.3137254902, green: 0.8117647059, blue: 0.8588235294, alpha: 1))
...
extension ChatViewController: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        let message = PMessage(user: user, content: text)
